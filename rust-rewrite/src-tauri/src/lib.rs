...
#[tauri::command]
fn undo_last_run(folder_path: String) -> Result<OrganizerStats, String> {
  let expanded = shellexpand::tilde(&folder_path).to_string();
...
fn get_default_path() -> String {
  ...
  // Fallback to default
  candidates.into_iter().next().unwrap_or_else(|| format!("{home}/Pictures/VRChat/VRChat"))
}
...
async fn start_watching(
  ...
  std::thread::spawn(move || {
    ...
    match organize_path(&initial_config, &mut stats) {
      Ok(()) => {
        let _ = initial_app_handle.emit("watch-initial-scan", serde_json::json!({
          "stats": stats
        }));
      }
      Err(e) => {
  ...
  // Notify frontend
  let _ = app.emit("watch-status", serde_json::json!({
    "status": "started",
  ...
  std::thread::spawn(move || {
    ...
    let mut watcher = match RecommendedWatcher::new(tx, Config::default()) {
      Ok(w) => w,
      Err(e) => {
        let _ = app_handle.emit("watch-error", serde_json::json!({"error": format!("Failed to create watcher: {}", e)}));
    ...
    if let Err(e) = watcher.watch(&watch_path_clone, RecursiveMode::Recursive) {
      let _ = app_handle.emit("watch-error", serde_json::json!({"error": format!("Failed to watch: {}", e)}));
      let state: State<WatcherState> = app_handle.state();
      state.running.store(false, Ordering::SeqCst);
      return;
    }
    ...
    *state.watcher.lock().unwrap() = Some(watcher);
    ...
    loop {
      ...
      // Every ~5 seconds (10 iterations at 500ms timeout), run a full poll
      poll_counter += 1;
      let should_poll = poll_counter >= 10;
      ...
      match rx.recv_timeout(Duration::from_millis(500)) {
        Ok(Ok(event)) => {
          ...
          for path in &event.paths {
            ...
            if let Err(e) = organize_path(&config_clone, &mut stats) {
              ...
              let _ = app_handle.emit("watch-organized", serde_json::json!({
                "stats": stats,
                "file": path.to_string_lossy()
              }));
            }
        ...
        Ok(Err(e)) => {
          let _ = app_handle.emit("watch-error", serde_json::json!({"error": e.to_string()}));
        }
      ...
      // Periodic poll: every ~5 seconds, scan for new files that might have been missed
      if should_poll {
        poll_counter = 0;
        ...
        if !new_files_found {
          let _ = app_handle.emit("watch-status", serde_json::json!({
            "status": "heartbeat",
